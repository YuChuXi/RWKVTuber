...
class RWKVTuberDataset(Dataset):
    ...
    def enconv_sample(self, sample):
        if sample[1].shape[0] < 2050:  # pad
            sample[0] = torch.nn.functional.pad(sample[0], [0, 0, 0, 4100])
            sample[1] = torch.nn.functional.pad(sample[1], [0, 0, 0, 2050])
            sample[2] = torch.nn.functional.pad(sample[2], [0, 0, 0, 1025])
        sample[0] = sample[0][:4100, ::].reshape(1025, 4 * 360)  # f0
        sample[1] = sample[1][:2050, ::].reshape(1025, 2 * 768)  # hubert
        sample[2] = sample[2][:1025, 1::]  # face
        #sample[3] = lrc_encoder(sample[3])
        #sample[3] = sample[3][:1025]  # logit
        ...
        return [
            torch.cat(sample[:-1], dim=-1),
            sample[3],
        ]  # f0 f0 f0 f0 face hubert hubert
